...
const Buster = () => {
    const group = useRef()
    const { nodes, materials, animations } = useGLTF("/models/buster_drone-transformed.glb")
    const { actions } = useAnimations(animations, group)
    console.log(actions);

    useEffect (() =>{
      
        actions['Start_Liftoff'].reset().fadeIn(.5).play();
...
useGLTF.preload("/models/buster_drone-transformed.glb")
